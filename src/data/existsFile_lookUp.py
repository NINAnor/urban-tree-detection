...
not_downloaded_filename = os.path.join(root, kommune,"lidar\laz", "inside_BuildUpZone","file_not_downloaded.csv")
...
with open(lookup_file, newline="") as csvfile:
    ...
    for row in reader:
        lookup_filenames.add(os.path.join(root, kommune, "lidar\laz", "inside_BuildUpZone", row[0]+".laz"))
...
for filename in lookup_filenames:
    if not os.path.exists(filename):
        ...
        with open(not_downloaded_filename, "a", newline="") as csvfile:
