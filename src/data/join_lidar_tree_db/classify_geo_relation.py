...
fc_case_1 = os.path.join(ds_urban_trees, "join_case_1") 
fc_case_2 = os.path.join(ds_urban_trees, "join_case_2") 
fc_case_3 = os.path.join(ds_urban_trees, "join_case_3")
fc_case_4 = os.path.join(ds_urban_trees, "join_case_4")
...
# Select case 4 and export to new fc 
if arcpy.Exists(fc_case_4):
    arcpy.AddMessage(f"\tFeature {os.path.basename(fc_case_4)} already exists. Continue...")
else:
    arcpy.SelectLayerByAttribute_management("lyr", "NEW_SELECTION", "{} IS NOT NULL AND {} IS NULL".format(field_crown_id, field_tree_id))
...
if arcpy.Exists(fc_case_3):
    ...
    arcpy.SelectLayerByAttribute_management("lyr", "NEW_SELECTION", "{} IS NULL AND {} IS NOT NULL".format(field_crown_id, field_tree_id))
